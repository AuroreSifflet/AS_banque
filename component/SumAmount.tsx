...
const CalculSolde = ({incommesArray, expencesArray, type}: SumAmountProps) => {
...
console.log(montant1Number); */
  ...
  const sumWithInitialIncomes = incommesArray.reduce(
    (previousValue: number, currentValue: incomes) =>
      previousValue + parseFloat(currentValue.amount.replace(/[€,]/g, '')),
    initialValue,
  );

  /* Somme des Expences */
  const sumWithInitialExpenses = expencesArray.reduce(
    (previousValue: number, currentValue: expenses) =>
      previousValue + parseFloat(currentValue.amount.replace(/[€,]/g, '')),
    initialValue,
  );

  /* Calcul du montant du compte Incomes - Expences */
  const SumAmount = sumWithInitialIncomes - sumWithInitialExpenses;

  /* Arrondir le solde du compte, la valeur de retour avec .fixed est une chaîne de caractère  */
  const SumAmountFormated = SumAmount.toFixed(2);

  //Buse, il faut téléchargé une libraire de formatage non utilisé - console.log(new Intl.NumberFormat('de-DE', { style: 'currency', currency: 'EUR' }).format(SumAmount));


  /* Créer une condition de type pour afficher une partie ou la totalité des informations. Exemple ici sur la page compte le composant est de type full et va afficher tous les éléments. Alors que sur la home nous n'affichons que le solde du compte.
  <></> est un raccourci pour <React.Fragment></React.Fragment> Il est différent de View car il ne crée pas d'élément DOM. 
  Cela permet d'englober les deux éléments textes avec un élément parent qui n'influe pas sur le comportement visuel de la page
  */
  return (
    <View>
      {type === 'full' && (
        <>
          <Text>
            Vos revenus sont de : {sumWithInitialIncomes.toFixed(2)} €
          </Text>
          <Text>
            Vos dépenses sont de : {sumWithInitialExpenses.toFixed(2)} €
          </Text>
        </>
      )}
